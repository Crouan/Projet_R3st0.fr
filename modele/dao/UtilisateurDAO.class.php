...
//            $stmt->bindValue(':mdpU', $mdpUCrypt, PDO::PARAM_STR);
            ...
            // Récupération du code d'erreur concernant le doublon du champ sur mailU
              if($e->getCode() == 23000) {
                  return false; // Retourne false pour signaler que le comprte existe 
              } else {
    ...
    public static function update(Utilisateur $unUser): bool {
        ...
        $stmt = Bdd::getConnexion()->prepare($requete);
        $mdpUCrypt = password_hash($unUser->getMdpU(), PASSWORD_BCRYPT);
        $stmt->bindValue(':mdpU', $mdpUCrypt, PDO::PARAM_STR);
//        $stmt->bindValue(':mdpU', $mdpUCrypt, PDO::PARAM_STR);
        $stmt->bindValue(':pseudoU', $unUser->getPseudoU(), PDO::PARAM_STR);
